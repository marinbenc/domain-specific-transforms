...
def get_model(dataset):
    # ITN uses the same model as seg, but the target is not a mask but the ITN transformed image
    model = seg.get_model(dataset.wrapped_dataset, sigmoid_activation=False)
    model.to('cuda')
    return model
...
def train_itn(batch_size, epochs, lr, dataset, subset, log_name):
    ...
    if p.exists(seg_path):
        ...
        model.load_state_dict(saved_seg['model'])
    else:
